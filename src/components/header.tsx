...
export function Header({
  ...
  let navbar: Record<string, { text: string; link: string; isTranslated?: boolean }[]> = finnishNavbar;

  if (langCode !== "fi") {
    navbar = Object.fromEntries(
      Object.entries(finnishNavbar).map(([header, items]) => {
        const innerHeader = NAVBAR_HEADER_MAP[langCode];
        const nativeHeader = innerHeader[header as keyof typeof innerHeader] ?? header;

        const nativeItems = items.map((item) => {
          const match = Object.values(nativeNavbar)
            .flat()
            .find(
              ({ link: nativeLink }) => nativeLink.slice(langCode.length + 1) === item.link.slice(3),
            );
          return {
  ...
  return (
    <>
      ...
      >
        <nav className="flex justify-between w-full items-center gap-4 lg:w-auto">
          <ul className="hidden h-full lg:flex">
            {Object.entries(navbar).map(([_header, children], idx) =>
        ...
        <AnimatePresence>
          {isOpen && (
            ...
            >
              {Object.entries(navbar).map(([_header, children]) =>
                children.map((child) => {
                  const url = pathname + child.link;
                  const isActive = currentPageMatch === child.link;
                  return (
              ...
              <div className="flex items-center gap-5">
                <button
                  onClick={openDialog}
                  className="hidden:lg lg:inline-flex h-9 items-center justify-center whitespace-nowrap rounded-md bg-u+green/50 bg-u+sunny px-3 pt-1 text-lg font-bold text-u+green ring-offset-background transition-colors hover:bg-u+sunny/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50"
                >
                  {langCode === "fi" ? "Ilmoittaudu" : "Sign Up"}
                </button>
      ...
      {/* Render the InfoPopup as an overlay outside the header */}
      {isDialogOpen && (
        <InfoPopup
