...
function TestForm() {
  ...
  function handleNextClick() {
    const allYesSelected = Object.values(answers).every(
      (answer) => answer === "Yes",
    );
  ...
  useEffect(() => {
    ...
    const anyRadioUnselected = Object.values(answers).some(
      (value) => value === "",
    );
  ...
  return (
    <div className="md:grid md:grid-cols-2" id="pink-box">
      <form className="md:p-18 pink-bg p-4">
        {showRadioForm && (
          <motion.div
            className="md:p-8"
            initial={{ opacity: 0 }}
          ...
          >
            ...
            <div className="mt-10 flex justify-end md:mt-4">
              <button
                className={`btn-form btn btn-outline btn-lg w-40 ${allRadioChecked ? "" : "cursor-not-allowed opacity-80"
                  }`}
